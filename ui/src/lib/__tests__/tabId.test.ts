...
function storage(initial: string | null = null): TabStorage & {
	value: string | null;
} {
	return {
		value: initial,
...
describe('browser tab identity', () => {
	it('uses getRandomValues when the production provider creates a new tab identity', () => {
		...
		const getTabId = createTabIdProvider(target);
	...
	it('reuses one valid session-scoped value without generating another', () => {
		...
		const getTabId = createTabIdProvider(target, createId);
	...
	it('replaces invalid stored data with one strictly opaque identifier', () => {
		...
		const getTabId = createTabIdProvider(target, () => 'tab-generated:1');
	...
	it('fails closed when the generator does not produce an opaque value', () => {
		const target = storage();
		const getTabId = createTabIdProvider(target, () => '../shared-tab');

		expect(() => getTabId()).toThrow('Generated tab identity is invalid');
		expect(target.getItem(TAB_STORAGE_KEY)).toBeNull();
	});
});

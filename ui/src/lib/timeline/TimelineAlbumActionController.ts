...
export type TimelineAlbumActionPhase =
	| 'idle'
	| 'resolving'
	| 'choosing'
	| 'executing'
...
export interface TimelineAlbumActionState {
	readonly phase: TimelineAlbumActionPhase;
...
export interface TimelineAlbumActionBeginInput {
	readonly albumLocalId: string;
	readonly zoneId: string;
	readonly tabId: string;
	/** Generation from the current live Timeline session. */
...
export type TimelineAlbumActionBeginResult =
...
export interface TimelineAlbumActionSocket extends BoundedAckSocket {
...
export interface TimelineAlbumActionControllerDependencies {
	readonly getSocket: () => TimelineAlbumActionSocket | null;
...
interface ActiveAttempt {
	...
	readonly socket: TimelineAlbumActionSocket;
...
/**
 * DOM-independent client state machine for the two-phase Timeline album-action
 ...
 */
export class TimelineAlbumActionController {
	readonly #getSocket: () => TimelineAlbumActionSocket | null;
	readonly #createRequestId: () => string;
	readonly #now: () => number;
	readonly #setTimer: (callback: () => void, milliseconds: number) => TimerHandle;
	...
	readonly #choosingTimeoutMs: number;
	readonly #executeAckTimeoutMs: number;
	readonly #subscribers = new Set<(state: TimelineAlbumActionState) => void>();

	#state: TimelineAlbumActionState;
	#attempt: ActiveAttempt | null = null;
	#disposed = false;

	constructor(dependencies: TimelineAlbumActionControllerDependencies) {
		this.#getSocket = dependencies.getSocket;
		this.#createRequestId = dependencies.createRequestId ?? createSecureTimelineOpaqueId;
	...
	subscribe(run: (state: TimelineAlbumActionState) => void): () => void {
		this.#subscribers.add(run);
		run(this.#state);
		return () => this.#subscribers.delete(run);
	}

	snapshot(): TimelineAlbumActionState {
		return this.#state;
	}

	begin(input: TimelineAlbumActionBeginInput): TimelineAlbumActionBeginResult {
	...
	quiesce(): void {
		...
		if (attempt.phase === 'resolving' || attempt.phase === 'choosing') {
			this.#cancelAttempt(attempt, 'QUIESCED', 'Album action canceled while Timeline quiesced');
			return;
		}
		this.#finish(
			attempt,
			...
			'Execution remains server-owned after Timeline quiesced'
		);
	}
	...
	#finish(
		...
		phase: Extract<TimelineAlbumActionPhase, 'executed' | 'failed' | 'canceled' | 'outcome-unknown'>,
		code: string | null,
		error: string | null
	): void {
	...
	#publishTerminal(
		phase: Extract<TimelineAlbumActionPhase, 'failed' | 'canceled' | 'outcome-unknown'>,
	...
	#idleState(): TimelineAlbumActionState {
	...
	#publish(values: Partial<TimelineAlbumActionState>): void {

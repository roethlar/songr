...
class FakeSocket implements AlbumActionSocket {
	connected = true;
...
function makeController(socket: FakeSocket, requestIds = ['request-1']) {
	let index = 0;
	return new AlbumActionController({
		getSocket: () => socket,
...
function reachChoosing(controller: AlbumActionController, socket: FakeSocket): void {
...
describe('AlbumActionController', () => {
	beforeEach(() => {
		vi.useFakeTimers();
	...
	it('sends one exact keyless begin and accepts only exact correlated choices', () => {
		...
		expect(socket.emission('album-action:begin').value).not.toHaveProperty('albumLocalId');
		socket.emission('album-action:begin').ack(accepted());
	...
	it('invokes browser-branded timer dependencies without a controller receiver', () => {
		...
		const controller = new AlbumActionController({
			getSocket: () => socket,

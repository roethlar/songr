...
import { TIMELINE_ALBUM_DETAIL_MAX_TRACKS } from "../../shared/timelineBrowseContracts";
import {
...
const MAX_DETAIL_ROWS =
  TIMELINE_ALBUM_DETAIL_MAX_TRACKS + ALBUM_ACTION_MAX_CHOICES;
const MAX_DUPLICATE_REVALIDATIONS = ALBUM_ACTION_MAX_CHOICES;
const CONTROL_CHARACTER = /\p{Cc}/u;

export type TimelineAlbumDetailResolverErrorCode =
  | "ALBUM_NOT_FOUND"
  | "ALBUM_AMBIGUOUS"
  | "DETAIL_INCOMPLETE"
  | "DETAIL_MISMATCH";

export class TimelineAlbumDetailResolverError extends Error {
  public constructor(
    public readonly code: TimelineAlbumDetailResolverErrorCode,
    message: string
  ) {
    super(message);
    this.name = "TimelineAlbumDetailResolverError";
    Object.setPrototypeOf(this, TimelineAlbumDetailResolverError.prototype);
    Error.captureStackTrace?.(this, TimelineAlbumDetailResolverError);
  }
}

export interface TimelineAlbumDetailResolution {
  readonly observation: ResolvedSelectedArtistObservation;
  readonly orderedTrackTitles: readonly string[];
}

export interface TimelineAlbumDetailParentObserver {
  observeCurrent(
...
export interface TimelineAlbumEditionCandidate {
...
export interface TimelineAlbumEditionDescriptor {
  readonly title: string;
  readonly artist: string;
  readonly editionText: string;
}
 ...
 */
export class TimelineAlbumDetailResolver {
  public constructor(
    private readonly parentObserver: TimelineAlbumDetailParentObserver =
      new TimelineDiscographyResolver()
  ) {}
  ...
  private readDetail(
    ...
    if (
      tracks.length < 1 ||
      tracks.length > TIMELINE_ALBUM_DETAIL_MAX_TRACKS ||

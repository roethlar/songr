...
export type DiscographyResolution =
  | {
 ...
 */
export interface ObservedDiscography {
  readonly observation: ResolvedSelectedArtistObservation;
  readonly liveAlbums: readonly TimelineLiveAlbumRow[];
}

export type DiscographyResolverErrorCode =
...
export class DiscographyResolverError extends Error {
  public constructor(
    public readonly code: DiscographyResolverErrorCode,
    message: string
  ) {
    super(message);
    this.name = "DiscographyResolverError";
    Object.setPrototypeOf(this, DiscographyResolverError.prototype);
    Error.captureStackTrace?.(this, DiscographyResolverError);
  }
}
  ...
  const code =
    label === "artist root"
      ...
      : label === "discography"
        ? "INCOMPLETE_DISCOGRAPHY"
        : "DISCOGRAPHY_PATH_NOT_UNIQUE";
  return new DiscographyResolverError(code, message);
}
  ...
  public async resolve(
    ...
    for (let depth = 0; depth <= TIMELINE_DISCOGRAPHY_MAX_DEPTH; depth += 1) {
      ...
      if (albumPaths.length !== 1) {
        throw new DiscographyResolverError(
          "DISCOGRAPHY_PATH_NOT_UNIQUE",

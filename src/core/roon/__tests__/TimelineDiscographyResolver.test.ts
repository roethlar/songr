import type { ArtistRef } from "../../../shared/timelineCatalogContracts";
import type {
  BrowseItem,
  BrowseLoadOptions,
  BrowseOptions,
...
import { TimelineDiscographyResolver } from "../TimelineDiscographyResolver";
...
describe("TimelineDiscographyResolver", () => {
  it("resolves a normalized-exact artist across pages and retains direct album order", async () => {
    ...
    const result = await new TimelineDiscographyResolver().resolve(
      session,
      artist()
    );
  ...
  it("never maps explicit track, composition, or other non-album list rows as albums", async () => {
    ...
    await expect(
      new TimelineDiscographyResolver().resolve(session, artist())
  ...
  it("accepts 500 albums plus one action header but rejects a 501st album", async () => {
    for (const albumCount of [500, 501]) {
      ...
      const pending = new TimelineDiscographyResolver().resolve(session, artist());
      if (albumCount === 500) {
  ...
  it("re-observes the complete retained album level with server-only live rows", async () => {
    ...
    const observed = await new TimelineDiscographyResolver().observeCurrent(
      session,
      artist()
    );
  ...
  it("rejects a retained current level that is not the expected discography", async () => {
    ...
    await expect(
      new TimelineDiscographyResolver().observeCurrent(session, artist())

...
import { TimelineObservedDiscography } from "../TimelineDiscographyResolver";
...
function observed(
  albums: TimelineObservedDiscography["observation"]["albums"] = [
...
describe("TimelineAlbumDetailResolver", () => {
  it("consumes one fresh live row and emits complete keyless detail evidence", async () => {
    ...
    const resolved = await new TimelineAlbumDetailResolver().resolve(
      session,
      artist(),
      album(),
      observed()
  ...
  it("fails closed before browsing a duplicate descriptor", async () => {
    ...
    await expect(
      new TimelineAlbumDetailResolver().resolve(
        session,
        artist(),
        album(),
  ...
  it("revalidates duplicate descriptors by fingerprint with fresh parent keys", async () => {
    ...
    const resolved = await new TimelineAlbumDetailResolver(observer).resolve(
      session,
      artist(),
      album({
  ...
  it("rejects zero or multiple duplicate fingerprint matches", async () => {
    ...
    for (const [tracks, code] of [
      ...
      await expect(
        new TimelineAlbumDetailResolver(observer).resolve(
          session,
          artist(),
          target,
          observed(
  ...
  it("uses newly observed parent authority even when Roon repeats an opaque key value", async () => {
    ...
    await new TimelineAlbumDetailResolver(observer).resolve(
      session,
      artist(),
      album({
        trackTitleFingerprint: createCatalogTrackTitleFingerprint(["Target"]),
  ...
  it("lists every matching live edition as a display-only candidate", () => {
    ...
    const candidates = new TimelineAlbumDetailResolver().observeCandidates(
      discography,
      album()
    );
  ...
  it("re-binds a chooser descriptor to exactly its edition without a fingerprint proof", async () => {
    ...
    const resolved = await new TimelineAlbumDetailResolver().resolveCandidate(
      session,
      artist(),
  ...
  it("fails closed when a descriptor matches zero or multiple live editions", async () => {
    const resolver = new TimelineAlbumDetailResolver();
    const gone = observed([
      { exactTitle: "Debut", exactArtist: "Björk", editionText: "" },
    ]);
    await expect(
  ...
  it("keeps header proofs for chooser re-binds", async () => {
    ...
    await expect(
      new TimelineAlbumDetailResolver().resolveCandidate(
        session,
        artist(),
        album(),
        observed(),
